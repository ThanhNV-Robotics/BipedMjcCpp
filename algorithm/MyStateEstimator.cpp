    ...
    bool verbose) // verbose is option to print out filter param or note
{
  ...
  // scale foot position noise
  this->Q_.block<3, 3>(6, 6) *= footProcessNoisePosition_;
...
void StateEstimator::update() {
  ...
  if (this->touch_lf >= 50) {
    this->leg_contac[0] = true; // set left leg contact flag to true
  } else {
    this->leg_contac[0] = false; // set left leg contact flag to false
  }
  if (this->touch_rf >= 50) {
    this->leg_contac[1] = true; // set left leg contact flag to true
  } else {
    this->leg_contac[1] = false; // set left leg contact flag to false
  }
}
