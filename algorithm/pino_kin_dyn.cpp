...
// this model, instead of pinocchio::Model::getJointId() silently returning an
// out-of-range sentinel (== njoints) that only surfaces as a crash much later,
// deep inside whichever algorithm first indexes with it.
pinocchio::JointIndex getJointIdChecked(const pinocchio::Model &model, const std::string &name)
{
    if (!model.existJointName(name))
    {
        throw std::runtime_error("Pin_KinDyn: joint '" + name + "' not found in URDF model");
    }
...
Pin_KinDyn::Pin_KinDyn(std::string urdf_pathIn)
{
    ...
    // get joint index for Pinocchio Lib -- names must match this project's
    // URDF (models/urdf/biped_robot_12dof.urdf), not OpenLoong's full-humanoid
    // naming. getJointIdChecked() throws immediately if a name doesn't exist,
    // instead of silently returning an out-of-range index that only crashes
    // later, deep inside whichever algorithm first uses it.
    // "ankle_joint"/"ankle_joint_fixed" is treated as the foot end-effector
    // frame throughout this class (IK target, foot Jacobians, fe_l/r_pos),
    // so it must point at the last joint before the actual foot link --
    // ankle_pitch, not ankle_roll (there's one more revolute joint after roll).
    r_ankle_joint = getJointIdChecked(model_biped, "right_ankle_pitch_joint");
    l_ankle_joint = getJointIdChecked(model_biped, "left_ankle_pitch_joint");
    r_hip_joint = getJointIdChecked(model_biped, "right_hip_roll_joint");
    l_hip_joint = getJointIdChecked(model_biped, "left_hip_roll_joint");
    r_hip_roll_joint = getJointIdChecked(model_biped, "right_hip_roll_joint");
    l_hip_roll_joint = getJointIdChecked(model_biped, "left_hip_roll_joint");
    r_ankle_joint_fixed = getJointIdChecked(model_biped_fixed, "right_ankle_pitch_joint");
    l_ankle_joint_fixed = getJointIdChecked(model_biped_fixed, "left_ankle_pitch_joint");
    r_hip_joint_fixed = getJointIdChecked(model_biped_fixed, "right_hip_roll_joint");
